...
export function formatJsonPreserve(src: string) {
    ...
    let out = '', depth = 0, lsOut = true, inValue = false;
...
export function Reveal({children, index, fill, active}: {
...
export function ToolActionBar({label, actions}: { label: string; actions: ToolBarAction[] }) {
    ...
    const commit = (next: string, opts?: { isolate?: boolean }) => {
        ...
        if (next === cur) return;
        const now = Date.now();
        if (!opts?.isolate && now - lastAt.current < 600 && past.current.length) past.current[past.current.length - 1] = cur; else {
