...
- item 由 `id`/`labelKey`/`groupKey`/`subgroupKey?`/`icon`/`keywords`/`tool?`/`action?`/`mode?`/`target?`/`pane?`/`needsInput?`/`whenSortOpen?`/`whenSchemaOpen?` 构成,定义在 `App.tsx`;索引 `buildIndex` 把 label + group + subgroup + keywords 一并纳入(含拼音/首字母),搜索「剪贴板」「大小写」等子分组词也要命中。
...
- **可切换面板的命令必须按状态可见**:属于某个可切换面板的操作命令只能在该面板打开时加入命令索引;例如 JSON Schema 右侧的「复制」「清空」「压缩」「格式化」必须使用 `whenSchemaOpen` 之类的状态条件,Schema 关闭时不可见。切换面板本身的命令(如「切换 Schema」)始终保留,用于打开或关闭面板。
